...
At beta=0.0 this recovers standard BPE. At beta=1.0 (this submission), leverage dominates. Both frequency and leverage are log-scaled before normalization. Breadth was computed but dropped from the final score (std=0.15, no discriminative power).
...
| Gravity beta=0.3 (70 swaps) | 2,870 | 1.3845 | **-0.017** |
| Cold replication beta=0.3 | 2,870 | 1.3821 | **-0.019** (confirms) |
...
```bash
# Setup: retokenize FineWeb with gravity tokenizer (see scripts/)
# Data must be in ./data/datasets/fineweb_gravity_beta_1.0/
# Tokenizer must be at ./data/tokenizers/gravity_beta_1.0.model

D_MODEL=384 NUM_LAYERS=12 NUM_HEADS=6 NUM_KV_HEADS=2 MLP_MULT=3 \
