...
from v43_metrics import (
    ...
    solver_ast_decomposition,
    solver_compression_ratio,
...
def append_cycle_snapshot(
    ...
    payload = {
        ...
        "solver_content": read_text(SOLVER_FILE),
        "dead_ends_json_content": read_text(DEAD_ENDS_JSON_FILE),
        "dead_end_state_content": read_text(DEAD_END_STATE_FILE),
...
def append_validation_event(
    *,
    ...
    with open(VALIDATION_LOG_FILE, "a", encoding="utf-8", newline="\n") as handle:
        handle.write(json.dumps(payload))
        handle.write("\n")


def append_invocation_event(payload: dict[str, object]) -> None:
    with open(INVOCATION_LOG_FILE, "a", encoding="utf-8", newline="\n") as handle:
        handle.write(json.dumps(payload))
        handle.write("\n")
...
def setup_workspace() -> None:
    ...
    if not os.path.exists(VALIDATION_LOG_FILE):
        write_text(VALIDATION_LOG_FILE, "")
    if not os.path.exists(INVOCATION_LOG_FILE):
        write_text(INVOCATION_LOG_FILE, "")
...
def enforce_workspace_valid(
    ...
    *,
    ...
    return final_error is None, final_error
...
def compute_cycle_metrics(
    ...
    metrics["solver_compression_ratio"] = solver_compression_ratio(attempted_solver_code)
    metrics.update(solver_ast_decomposition(attempted_solver_code))
    ...
    metrics.update(
        rolling_pink_metrics(
            [entry.get("solver_compression_ratio", 0) for entry in history_plus],
            "solver_compression",
        )
    )
    ...
    while cycle < target_cycles:
        ...
        if not valid:
            last_error = (
                ...
                + (f" Last validator error: {validation_error}" if validation_error else "")
            )[:1000]
